...
MATERIAL_RAG_SERVICE_PATH = Path(__file__).resolve().parents[1] / "app" / "material_rag_service.py"
...
def test_material_rag_service_imports_domain_models_only():
    tree = ast.parse(MATERIAL_RAG_SERVICE_PATH.read_text(encoding="utf-8"), filename=str(MATERIAL_RAG_SERVICE_PATH))

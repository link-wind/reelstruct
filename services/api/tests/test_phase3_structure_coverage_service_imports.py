...
STRUCTURE_COVERAGE_SERVICE_PATH = Path(__file__).resolve().parents[1] / "app" / "structure_coverage_service.py"
...
def test_structure_coverage_service_imports_domain_models_only():
    tree = ast.parse(STRUCTURE_COVERAGE_SERVICE_PATH.read_text(encoding="utf-8"), filename=str(STRUCTURE_COVERAGE_SERVICE_PATH))

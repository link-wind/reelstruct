...
MATERIAL_RERANK_SERVICE_PATH = Path(__file__).resolve().parents[1] / "app" / "material_rerank_service.py"
...
def test_material_rerank_service_imports_domain_models_only():
    tree = ast.parse(MATERIAL_RERANK_SERVICE_PATH.read_text(encoding="utf-8"), filename=str(MATERIAL_RERANK_SERVICE_PATH))

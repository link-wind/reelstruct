...
| 结构拆解 | 拆出 Hook、卖点展开、使用过程、CTA；生成节奏摘要和包装信号 | 已完成 P0 |
...
| 迁移过程可视化 | 前端展示样例解析、结构槽位映射、缺口、补位策略、素材来源和生成 trace | 已完成 P0 |
...
| 真实素材适配 | 支持上传并绑定到结构槽位，渲染时优先使用；尚未做镜头分类、高光筛选、主体识别 | 部分完成 |
...
当前系统可以上传真实素材并绑定槽位，但还没有真正理解素材内容。下一步可以加入：
...
2. 再讲方法：把样例拆成 `TemplateStructure`，迁移成 `TransferPlan`，缺口落到 `MaterialGap`，最终生成 `CompositionSpec`。
